...
class Tile(AbstractTile):
    ...
    def add_face(self, index_keys, vert_index_keys_list, **kwargs):
        ...
        if self._get_face(index_keys) is not None:
            # print('not calling create face, face is not None')
            return None

        verts = self._get_verts_from_list(vert_index_keys_list)
        if verts is None:
            # print('not calling create face, verts is None')
            return None

        # print('calling create face')
    ...
    def _get_vert(self, index_keys):
        # print('trying to get %s from self.verts' % index_keys)
    ...
    def _get_verts_from_list(self, vert_index_keys_list):
        verts = []
        # print('trying to get verts from', vert_index_keys_list)
        for vert_index_keys in vert_index_keys_list:
            # print('looking at', vert_index_keys)
            if isinstance(vert_index_keys[0], list):
                # print('the first one was a list')
                vert = self._get_neighbor_vert(vert_index_keys[0],
                                               vert_index_keys[1])
            else:
                # print('the first one was not a list')
        ...
        # print('returing %s verts' % len(verts))
        return verts
