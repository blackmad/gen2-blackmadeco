...
class AbstractTile(ValueBlend):
    ...
    def _get_nested_list_value(self, nested_list, index_keys):
        ...
        if not isinstance(index_keys, list):
            # print('is not instance of list, so just grabbing from nested list')
            return nested_list[index_keys]
        value = nested_list
        # print('looking for %s' % index_keys)
        for index in index_keys:
            # print('looking for %s' % index)
            value = value[index]
        # print('returning', value)
        return value

    def _set_nested_list_value(self, nested_list, index_keys, value):
        # print('setting nested list value', nested_list, index_keys, value)
        if not isinstance(index_keys, list):
            # print('is not a list, so just setting')
            nested_list[index_keys] = value
            return
        reference = nested_list
        for index in index_keys[0:-1]:
            reference = reference[index]
        # print('new ref', reference)
        # print('setting at', index_keys[-1])
        key = index_keys[len(index_keys) - 1]
        # print('setting at', key)
        # print('to', value)
        reference[key] = value
